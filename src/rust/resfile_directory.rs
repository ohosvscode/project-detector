...
impl ResfileDirectory {
  #[napi]
  pub fn from(resource: Reference<Resource>) -> Option<ResfileDirectory> {
    let uri = Uri::file(Path::new(&resource.get_uri().fs_path()).join("resfile").to_string_lossy().to_string());
    if !fs::metadata(uri.fs_path()).map(|metadata| metadata.is_dir()).unwrap_or(false) {
      return None;
    }

    Some(ResfileDirectory { uri, resource })
  }
